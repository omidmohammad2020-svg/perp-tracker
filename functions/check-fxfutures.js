...
// No indicators. The first 15-minute candle of each UTC trading day
// (00:00-00:15) defines the opening range. A close beyond that range's
// high/low triggers entry (once per instrument per day). Stop-loss is the
// opposite side of the opening range. Take-profit targets the previous UTC
// day's high (longs) or low (shorts).
...
function computeOpeningRange(candles) {
  ...
  const openingCandle = candles.find(c => c.time >= todayMidnight && c.time < todayMidnight + 15 * 60 * 1000);
  if (!openingCandle) return null;

  const prevDayStart = todayMidnight - 24 * 60 * 60 * 1000;
  const prevDayCandles = candles.filter(c => c.time >= prevDayStart && c.time < todayMidnight);
  if (!prevDayCandles.length) return null;

  return {
    openingHigh: openingCandle.high,
    openingLow: openingCandle.low,
...
async function fetchTwelveDataSeries(symbol) {
  ...
  const url = `https://api.twelvedata.com/time_series?symbol=${encodeURIComponent(symbol)}&interval=15min&outputsize=250&apikey=${TD_API_KEY}`;
...
async function fetchYahooSeries(symbol) {
  const url = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?interval=15m&range=5d`;
  const res = await fetch(url, {
  ...
  for (let i = 0; i < result.timestamp.length; i++) {
    ...
    candles.push({ high: quote.high[i], low: quote.low[i], close: quote.close[i], time: result.timestamp[i] * 1000 });
  }
  return candles;
}
...
const STATE_VERSION = 2; // ORB strategy replaces market structure

exports.handler = async function () {
  const store = getFxFuturesStore();
  let state = (await store.get('state', { type: 'json' })) || { trades: {}, latest: {}, tradedToday: {} };
  ...
  if (state.version !== STATE_VERSION) {
    console.log(`Resetting FX & Futures tracking data — new strategy version (${STATE_VERSION}).`);
    state = { trades: {}, latest: {}, tradedToday: {}, hourlyCount: { hour: getHourBucket(), count: 0 }, version: STATE_VERSION };
    ...
    await sendTelegram(
      `🔄 <b>Tracker reset</b> — new strategy live: Opening Range Breakout (ORB). The first 15-minute candle of ` +
      `each UTC trading day (00:00-00:15) sets the range; a close beyond that range's high/low triggers entry ` +
      `(max once per instrument per day). Stop-loss is the opposite side of the range, take-profit targets the ` +
      `previous day's high/low. No indicators. Max ${MAX_TRADES_PER_HOUR} trades per hour combined across all ` +
      `${INSTRUMENTS.length} instruments. Starting fresh from now.`
    );
  }
  ...
  await Promise.all(INSTRUMENTS.map(async (instrument) => {
    ...
    try {
      ...
      state.latest[symbol] = { label, source, price, updatedAt: Date.now() };
      ...
      if (ts && ts.status === 'active') {
        ...
        const range = computeOpeningRange(candles);

        if (range && state.tradedToday[symbol] !== range.todayMidnight) {
          let direction = null, entry = null, sl = null, tp = null;

          if (lastCandle.close > range.openingHigh && range.prevDayHigh > price) {
            direction = 'LONG'; entry = price; sl = range.openingLow; tp = range.prevDayHigh;
          } else if (lastCandle.close < range.openingLow && range.prevDayLow < price) {
            direction = 'SHORT'; entry = price; sl = range.openingHigh; tp = range.prevDayLow;
          }
          ...
          if (direction) {
            ...
            if (risk > 0 && reward > 0 && state.hourlyCount.count < MAX_TRADES_PER_HOUR) {
              ...
              state.trades[symbol] = {
                status: 'active', direction, entry, sl, tp, rr, strength,
                leverage, initialRisk, breakevenMoved: false, openedAt: Date.now()
              };
              state.tradedToday[symbol] = range.todayMidnight;
              state.hourlyCount.count++;
              weekly.signals++;
              await sendTelegram(
                `📡 <b>${label} ${direction}</b> (${strength}/10)\n` +
                `Opening Range Breakout — closed beyond the ${direction === 'LONG' ? 'range high' : 'range low'} ` +
                `(${fmt(direction === 'LONG' ? range.openingHigh : range.openingLow)})\n` +
                `Entry: ${fmt(entry)}\nSL: ${fmt(sl)} (opposite side of the range)\nTP: ${fmt(tp)} (previous day's ${direction === 'LONG' ? 'high' : 'low'})\nR:R 1:${rr}\n` +
                `Suggested leverage: ${leverage}x (based on setup confidence)\n` +
                `Trade ${state.hourlyCount.count}/${MAX_TRADES_PER_HOUR} this hour`,
                source
              );
              log({ sym: label, type: 'SIGNAL', direction, strength, entry, sl, tp });
            } else if (risk > 0 && reward > 0) {
              console.log(`${label}: valid ORB breakout but hourly cap (${MAX_TRADES_PER_HOUR}) already reached — skipped for today.`);
              state.tradedToday[symbol] = range.todayMidnight;
            }
          }
        }
