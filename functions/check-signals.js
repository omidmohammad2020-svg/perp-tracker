...
async function sendTelegram(text) {
  ...
  } catch (e) { /* alert failed silently — doesn't block the rest of the run */ }
}
  ...
  for (const sym of SYMBOLS) {
    try {
      ...
      if (ts && ts.status === 'active') {
        ...
        if (hit) {
          ts.status = 'closed'; ts.outcome = hit; ts.closePrice = price;
          ...
          // emergency invalidation, debounced over 2 consecutive runs (10 min)
          let flipped = false;
          ...
          if (ts.flipStrikes >= 2 || ts.volStrikes >= 2) {
            ts.status = 'emergency';
            weekly.invalidated++;
            const reason = ts.flipStrikes >= 2 ? 'trend reversed and held' : 'volatility spike (ATR tripled) and held';
        ...
        if (result.signal === 'LONG' || result.signal === 'SHORT') {
          state.trades[sym] = {
            ...
            atrAtOpen: result.atrNow, trendUpAtOpen: result.trendUp
          };
          weekly.signals++;
          await sendTelegram(
        ...
        } else if (result.signal === 'WATCH' && [5, 8, 9, 10].includes(result.signalStrength)) {
          if (state.watchAlerts[sym] !== result.watchDirection) {
            state.watchAlerts[sym] = result.watchDirection;
            await sendTelegram(`👀 <b>${sym} WATCH ${result.watchDirection}</b> (${result.signalStrength}/10 partial)`);
      ...
      // one symbol failing (e.g. transient API hiccup) shouldn't stop the rest
    }
  }
