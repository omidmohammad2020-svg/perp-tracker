...
exports.handler = async function () {
  ...
  if (state.version !== STATE_VERSION) {
    ...
    state = { trades: {}, pendingBreakouts: {}, version: STATE_VERSION };
    ...
    await sendTelegram('🔄 <b>Tracker reset</b> — new crypto-only strategy live: wedge/apex breakout + retest. Converging support/resistance trendlines from swing highs/lows, trade only fires after a confirmed retest of the broken line, target = measured move (wedge height projected from the breakout point). No indicators. Starting fresh from now.');
  }
        ...
        if (!pending) {
          ...
          if (retested) {
            ...
            if (risk > 0 && reward > 0) {
              ...
              weekly.signals++;
              console.log(`${sym}: ${pending.direction} confirmed retest — strength ${strength}/10, R:R 1:${rr}`);
              await sendTelegram(
