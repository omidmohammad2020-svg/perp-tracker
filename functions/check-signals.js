    ...
    try {
      ...
      if (ts && ts.status === 'active') {
        ...
        if ((result.signal === 'LONG' || result.signal === 'SHORT') && state.hourlyCount.count < MAX_TRADES_PER_HOUR) {
          ...
          const liqDistPct = Math.round((100 / result.leverage) * 10) / 10;
          state.trades[sym] = {
          ...
          weekly.signals++;
          console.log(`${sym}: ${result.signal} trend continuation — strength ${result.strength}/10, ${result.leverage}x (${state.hourlyCount.count}/${MAX_TRADES_PER_HOUR} this hour)`);
          await sendTelegram(
            `📡 <b>${sym} ${result.signal}</b> (${result.strength}/10)\n` +
            `Trend continuation — breakout with rising volume, in the direction of the 50-period average\n` +
            `Entry: $${fmt(result.price)}\nSL: $${fmt(result.sl)} (real swing level)\nTP: $${fmt(result.tp)} (2:1 R:R)\n` +
            `Leverage: ${result.leverage}x — liquidation ≈${liqDistPct}% away, stop ≈${Math.round((initialRisk / result.price) * 1000) / 10}% away\n` +
          ...
          console.log(`${sym}: valid ${result.signal} but hourly cap (${MAX_TRADES_PER_HOUR}) already reached — skipped, not queued.`);
        }
      }
    } catch (e) {
