    ...
    try {
      ...
      if (ts && ts.status === 'active') {
        ...
        if (hit) {
          ...
          // emergency invalidation, debounced over 10 consecutive runs (~10 min at
          // the 1-minute check interval — same real-world confirmation window as
          // before, just scaled for the faster cadence)
          const CONFIRM_STRIKES = 10;
          let flipped = false;
