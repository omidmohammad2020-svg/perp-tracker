...
// Fires true exactly once per 3-hour UTC window (00:00, 03:00, 06:00, ...) — the
// first 5-minute-scheduled run after each mark catches minute < 5 and sends the
// heartbeat, every run after that in the same window sees minute >= 5 and skips.
function isHeartbeatWindow() {
  const now = new Date();
  return now.getUTCHours() % 3 === 0 && now.getUTCMinutes() < 5;
}

// Fires true once per day at 08:00 UTC = 12:00 UAE time (UTC+4, no DST).
...
exports.handler = async function () {
  ...
  let daily = (await store.get('daily', { type: 'json' })) || { date: todayStr, totalPnlPct: 0, count: 0 };
  if (daily.date !== todayStr) daily = { date: todayStr, totalPnlPct: 0, count: 0 };
  ...
  // Wall-clock aligned (fires once in the first 5-min run after each 3-hour UTC
  // mark: 00:00, 03:00, 06:00...) rather than "3 hours since last sent" — this
  // keeps it naturally synced with the forex tracker's heartbeat, since both run
  // on the same 5-minute schedule and will hit this same window together.
    ...
    try {
      ...
      if (ts && ts.status === 'active') {
        ...
        if (hit) {
          ...
          const TREND_FLIP_BUFFER = 0.15; // % beyond the EMA required to count as a
                                           // real reversal, not a whisker cross near it
          let flipped = false;
          ...
          if (ts.flipStrikes >= CONFIRM_STRIKES || ts.volStrikes >= CONFIRM_STRIKES) {
            ...
            const pnlText = `${pnlEmoji} ${pnlPct >= 0 ? '+' : ''}${pnlPct.toFixed(2)}%`;
            await sendTelegram(`🚨 <b>${sym} ${ts.direction} INVALIDATED</b>\n${reason}. Close manually now.\nP/L at this point: ${pnlText}`);
            log({ sym, type: 'INVALIDATED', direction: ts.direction, price, pnlPct });
          }
        }
      } else {
        ...
        // WATCH tier no longer alerts — only confirmed signals, TP/SL, invalidation,
        // heartbeat, and the daily P/L recap send Telegram messages now.
      }
    } catch (e) {
  ...
  // Daily P/L recap — 08:00 UTC = 12:00 UAE time (UTC+4, no DST). Fires once in
  // the first 5-min run after that mark, same debounce pattern as the heartbeat.
  if (isDailyRecapWindow()) {
    ...
    daily = { date: todayStr, totalPnlPct: 0, count: 0 };
  }
