...
// Uptrend = higher highs AND higher lows (the last two confirmed pivots of
// each type). Downtrend = lower lows AND lower highs. Anything else = no
// clear structure, and the strategy simply doesn't trade it.
...
function computeSignal(candles) {
  ...
  if (structure.trend === 'UP') {
    // Demand zone = the base (the pivot-low candle's body) that preceded the
    // impulsive move up to the current higher high — buy the retest of it.
    const zoneCandle = structure.lastLow.candle;
    const zoneLow = zoneCandle.low;
    const zoneHigh = Math.max(zoneCandle.open, zoneCandle.close);
    const touchingZone = lastCandle.low <= zoneHigh && lastCandle.close > zoneLow;
    if (touchingZone) {
      const stop = zoneLow - atrNow * 0.1;
      const target = structure.lastHigh.price; // next resistance = prior swing high
    ...
    // Supply zone = the base that preceded the impulsive move down to the
    // current lower low — sell the retest of it.
    const zoneCandle = structure.lastHigh.candle;
    const zoneHigh = zoneCandle.high;
    const zoneLow = Math.min(zoneCandle.open, zoneCandle.close);
    const touchingZone = lastCandle.high >= zoneLow && lastCandle.close < zoneHigh;
    if (touchingZone) {
      const stop = zoneHigh + atrNow * 0.1;
      const target = structure.lastLow.price; // next support = prior swing low
  ...
  await Promise.all(SYMBOLS.map(async (sym) => {
    try {
      ...
      if (result.signal !== 'WAIT') {
        console.log(`${sym}: ${result.signal} — strength ${result.strength}/10, R:R 1:${result.rr}`);
      }
      ...
      if (ts && ts.status === 'active') {
        ...
        // its favor, move the stop to entry. Fixes the asymmetry from the
        // 10%+ TP floor — a tight SL next to a stretched TP means price is
        // statistically more likely to hit the close target first, so a
        // trade that was winning shouldn't be allowed to round-trip all the
        // way back to a full loss.
        ...
        // No WATCH tier in this strategy — either a valid demand/supply zone
        // retest with 2.5:1+ R:R exists right now, or it doesn't (WAIT).
        // Invalidation is gone — trades only ever resolve via TP or SL.
      }
    } catch (e) {
