...
const SYMBOLS = [
  ...
  'EOSUSDT', 'XTZUSDT', 'ALGOUSDT', 'VETUSDT', 'ICPUSDT', 'RUNEUSDT', 'GRTUSDT',
  'SANDUSDT', 'MANAUSDT', 'AAVEUSDT', 'MKRUSDT', 'CRVUSDT', 'LDOUSDT', 'RENDERUSDT',
  'HBARUSDT', 'THETAUSDT', 'KAVAUSDT', 'ZECUSDT', 'DASHUSDT', 'SEIUSDT', 'WLDUSDT'
];
...
// P/L as a dollar figure on a hypothetical $100 stake per trade. On $100,
// dollar gain/loss = 100 * (pnlPct/100) = pnlPct — numerically identical to
// the percentage, just displayed as a dollar amount instead.
function fmtDollarPnl(pnlPct) {
  const sign = pnlPct >= 0 ? '+' : '-';
  return `${sign}$${Math.abs(pnlPct).toFixed(2)}`;
}
      ...
      if (ts && ts.status === 'active') {
        ...
        if (hit) {
          ...
          if (ts.flipStrikes >= CONFIRM_STRIKES || ts.volStrikes >= CONFIRM_STRIKES) {
            ...
            const pnlText = `${pnlEmoji} ${fmtDollarPnl(pnlPct)}`;
