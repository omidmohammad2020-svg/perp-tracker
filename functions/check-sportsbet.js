...
const MAX_LEGS = 3;
const MAX_LEG_ODDS = 1.35; // ~74%+ implied probability — raised slightly from 1.25 so 3 legs can actually reach ~2x combined
...
exports.handler = async function () {
  ...
  if (!pending) {
    try {
      ...
      // Safest legs first, adding until combined odds cross ~2x (capped at 3).
      ...
      for (const c of candidates) {
        ...
        legs.push(c);
        combinedSoFar *= c.price;
        if (combinedSoFar >= 2.0) break;
      }
